...
현재 BGM은 `public/music/romantic-wedding-piano.mp3`이며, 설정은 `src/lib/music-config.ts`에서 관리합니다. 이전 오리지널 곡 `public/music/first-light.mp3`도 교체용으로 보존되어 있습니다. 페이지 왼쪽 아래의 **MUSIC OFF / MUSIC ON** 버튼으로 실제 재생과 일시정지를 할 수 있으며 음악은 반복 재생됩니다.
...
스마트폰 브라우저는 소리 있는 자동 재생을 제한하므로, 하객이 버튼을 누를 때 음악이 시작됩니다. 이 방식은 갑작스러운 소리도 피할 수 있습니다. 현재 파일은 사용자가 제공한 음원이므로 원본 다운로드 페이지와 라이선스 기록을 함께 보관하세요. 상업 OST 등을 공개하려면 해당 음악과 음원 사용에 필요한 허락을 먼저 확인해야 합니다. 선택적으로 오리지널 곡을 다시 만들려면 `scripts/generate-bgm.py`를 사용할 수 있지만, 사이트 실행과 배포에는 Python이 필요하지 않습니다.
